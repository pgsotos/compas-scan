...
class BrandContext(BaseModel):
    ...
    keywords: List[str] = Field(
        ...
        description="Extracted keywords from brand's website"
    )
...
class CompetitorCandidate(BaseModel):
    ...
    gemini_type: Optional[Literal["HDA", "LDA"]] = Field(
        None,
        description="Type suggested by Gemini (if from AI)"
    )
...
class ClassificationResult(BaseModel):
    ...
    type: Optional[Literal["HDA", "LDA"]] = Field(
        None,
        description="Competitor type if valid (High/Low Domain Authority)"
    )
    ...
    justification: Optional[str] = Field(
        None,
        description="Justification for classification if valid"
    )
...
class ScanResponse(BaseModel):
    ...
    warnings: Optional[List[str]] = Field(
        None,
        description="Non-critical warnings during the process"
    )

...
from api.models import CompetitorCandidate, BrandContext
...
def test_invalid_netloc_is_handled():
    ...
    assert any(
        "Invalid URL structure" in d.reason or
        "empty domain" in d.reason or
        "Empty or invalid URL" in d.reason or
        d.url == "http://"  # At minimum, it should be discarded
        for d in discarded
    )
